...
const data = reactive<PageData<BomComponentForm, BomComponentQuery>>({
  ...
  rules: {
    ...
    bomBo: [{ required: true, message: '物料清单行号不能为空', trigger: 'blur' }],
    sequence: [{ required: true, message: '顺序号不能为空', trigger: 'blur' }],
    componentGbo: [{ required: true, message: '组件料号行号不能为空', trigger: 'blur' }],
    bomComponentType: [{ required: true, message: '物料组件类型不能为空', trigger: 'change' }],
    useBomDefaults: [{ required: true, message: '启用物料清单替代料不能为空', trigger: 'blur' }],
    assyDataTypeBo: [{ required: true, message: '装配数据类型行号不能为空', trigger: 'blur' }],
    qty: [{ required: true, message: '数量不能为空', trigger: 'blur' }],
    assemblyOperationBo: [{ required: true, message: '组件工序行号不能为空', trigger: 'blur' }],
    maximumUsage: [{ required: true, message: '组件最大使用次数不能为空', trigger: 'blur' }],
    refDes: [{ required: true, message: '参考点不能为空', trigger: 'blur' }],
