<template>
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="编号定义方式" prop="definedBy">
              <el-input v-model="queryParams.definedBy" placeholder="请输入编号定义方式" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="编码对象" prop="contextBo">
              <el-input v-model="queryParams.contextBo" placeholder="请输入编码对象" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="描述" prop="description">
              <el-input v-model="queryParams.description" placeholder="请输入描述" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="立即提交编号规则更改, true|false" prop="commitImmediately">
              <el-input v-model="queryParams.commitImmediately" placeholder="请输入立即提交编号规则更改, true|false" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="样例" prop="example">
              <el-input v-model="queryParams.example" placeholder="请输入样例" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="条码产生最后提交时间" prop="lastCommitDate">
              <el-date-picker clearable
                v-model="queryParams.lastCommitDate"
                type="date"
                value-format="YYYY-MM-DD"
                placeholder="请选择条码产生最后提交时间"
              />
            </el-form-item>
            ...
            <el-form-item label="主数据修改时间" prop="dataModifyTime">
              <el-date-picker clearable
                v-model="queryParams.dataModifyTime"
                type="date"
                value-format="YYYY-MM-DD"
                placeholder="请选择主数据修改时间"
              />
            </el-form-item>
            <el-form-item label="主数据修改用户" prop="dataModifyUser">
              <el-input v-model="queryParams.dataModifyUser" placeholder="请输入主数据修改用户" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:number:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:number:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:number:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:number:export']">导出</el-button>
          </el-col>
      ...
      <el-table v-loading="loading" :data="numberList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="编号定义类型" align="center" prop="nextNumberType" />
        <el-table-column label="编号定义方式" align="center" prop="definedBy" />
        <el-table-column label="编码对象" align="center" prop="contextBo" />
        <el-table-column label="描述" align="center" prop="description" />
        <el-table-column label="立即提交编号规则更改, true|false" align="center" prop="commitImmediately" />
        <el-table-column label="样例" align="center" prop="example" />
        <el-table-column label="条码产生最后提交时间" align="center" prop="lastCommitDate" width="180">
          <template #default="scope">
            <span>{{ parseTime(scope.row.lastCommitDate, '{y}-{m}-{d}') }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="主数据修改时间" align="center" prop="dataModifyTime" width="180">
          <template #default="scope">
            <span>{{ parseTime(scope.row.dataModifyTime, '{y}-{m}-{d}') }}</span>
          </template>
        </el-table-column>
        <el-table-column label="主数据修改用户" align="center" prop="dataModifyUser" />
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:number:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:number:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="numberFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="数据行索引" prop="handle">
          <el-input v-model="form.handle" placeholder="请输入数据行索引" />
        </el-form-item>
        <el-form-item label="编号定义方式" prop="definedBy">
          <el-input v-model="form.definedBy" placeholder="请输入编号定义方式" />
        </el-form-item>
        <el-form-item label="编码对象" prop="contextBo">
          <el-input v-model="form.contextBo" placeholder="请输入编码对象" />
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input v-model="form.description" placeholder="请输入描述" />
        </el-form-item>
        <el-form-item label="立即提交编号规则更改, true|false" prop="commitImmediately">
          <el-input v-model="form.commitImmediately" placeholder="请输入立即提交编号规则更改, true|false" />
        </el-form-item>
        <el-form-item label="样例" prop="example">
          <el-input v-model="form.example" placeholder="请输入样例" />
        </el-form-item>
        <el-form-item label="条码产生最后提交时间" prop="lastCommitDate">
          <el-date-picker clearable
            v-model="form.lastCommitDate"
            type="datetime"
        ...
        <el-form-item label="主数据修改用户" prop="dataModifyUser">
          <el-input v-model="form.dataModifyUser" placeholder="请输入主数据修改用户" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { listNumber, getNumber, delNumber, addNumber, updateNumber } from '@/api/mes/number';
import { NumberVO, NumberQuery, NumberForm } from '@/api/mes/number/types';
...
const data = reactive<PageData<NumberForm, NumberQuery>>({
  form: {...initFormData},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
  ...
  rules: {
    ...
    nextNumberType: [
      { required: true, message: "编号定义类型不能为空", trigger: "change" }
    ],
    definedBy: [
      { required: true, message: "编号定义方式不能为空", trigger: "blur" }
    ],
    contextBo: [
      { required: true, message: "编码对象不能为空", trigger: "blur" }
    ],
    description: [
      { required: true, message: "描述不能为空", trigger: "blur" }
    ],
    commitImmediately: [
      { required: true, message: "立即提交编号规则更改, true|false不能为空", trigger: "blur" }
    ],
    example: [
      { required: true, message: "样例不能为空", trigger: "blur" }
    ],
    lastCommitDate: [
      { required: true, message: "条码产生最后提交时间不能为空", trigger: "blur" }
    ],
    remark: [
    ...
    dataModifyTime: [
      { required: true, message: "主数据修改时间不能为空", trigger: "blur" }
    ],
    dataModifyUser: [
      { required: true, message: "主数据修改用户不能为空", trigger: "blur" }
    ]
  }
});
...
const handleUpdate = async (row?: NumberVO) => {
  ...
  const res = await getNumber(_id);
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = "修改编号规则定义主表";
}
...
const submitForm = () => {
  numberFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await updateNumber(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        await addNumber(form.value).finally(() =>  buttonLoading.value = false);
      }
...
const handleDelete = async (row?: NumberVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除编号规则定义主表编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delNumber(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('mes/number/export', {
    ...queryParams.value
