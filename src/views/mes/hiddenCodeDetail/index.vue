<template>
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="隐码组行号" prop="hiddenCodeGroupBo">
              <el-input v-model="queryParams.hiddenCodeGroupBo" placeholder="请输入隐码组行号" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            ...
            <el-form-item label="明码值" prop="plainCode">
              <el-input v-model="queryParams.plainCode" placeholder="请输入明码值" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="隐码值" prop="hiddenCode">
              <el-input v-model="queryParams.hiddenCode" placeholder="请输入隐码值" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:hiddenCodeDetail:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:hiddenCodeDetail:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:hiddenCodeDetail:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:hiddenCodeDetail:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="hiddenCodeDetailList" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="center" />
        <el-table-column label="记录唯一ID" align="center" prop="id" v-if="true" />
        <el-table-column label="隐码组行号" align="center" prop="hiddenCodeGroupBo" />
        <el-table-column label="顺序号" align="center" prop="sequence" />
        <el-table-column label="明码值" align="center" prop="plainCode" />
        <el-table-column label="隐码值" align="center" prop="hiddenCode" />
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:hiddenCodeDetail:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:hiddenCodeDetail:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="hiddenCodeDetailFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="隐码组行号" prop="hiddenCodeGroupBo">
          <el-input v-model="form.hiddenCodeGroupBo" placeholder="请输入隐码组行号" />
        </el-form-item>
        <el-form-item label="顺序号" prop="sequence">
          <el-input v-model="form.sequence" placeholder="请输入顺序号" />
        </el-form-item>
        <el-form-item label="明码值" prop="plainCode">
          <el-input v-model="form.plainCode" placeholder="请输入明码值" />
        </el-form-item>
        <el-form-item label="隐码值" prop="hiddenCode">
          <el-input v-model="form.hiddenCode" placeholder="请输入隐码值" />
        </el-form-item>
        <el-form-item label="所属组织ID" prop="belongOrgId">
...
<script setup name="HiddenCodeDetail" lang="ts">
import { listHiddenCodeDetail, getHiddenCodeDetail, delHiddenCodeDetail, addHiddenCodeDetail, updateHiddenCodeDetail } from '@/api/mes/hiddenCodeDetail';
import { HiddenCodeDetailVO, HiddenCodeDetailQuery, HiddenCodeDetailForm } from '@/api/mes/hiddenCodeDetail/types';

const { proxy } = getCurrentInstance() as ComponentInternalInstance;

const hiddenCodeDetailList = ref<HiddenCodeDetailVO[]>([]);
...
const hiddenCodeDetailFormRef = ref<ElFormInstance>();
...
const initFormData: HiddenCodeDetailForm = {
  id: undefined,
  hiddenCodeGroupBo: undefined,
  sequence: undefined,
  plainCode: undefined,
...
const data = reactive<PageData<HiddenCodeDetailForm, HiddenCodeDetailQuery>>({
  form: {...initFormData},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
  ...
  rules: {
    ...
    hiddenCodeGroupBo: [
      { required: true, message: "隐码组行号不能为空", trigger: "blur" }
    ],
    sequence: [
      { required: true, message: "顺序号不能为空", trigger: "blur" }
    ],
    plainCode: [
      { required: true, message: "明码值不能为空", trigger: "blur" }
    ],
    hiddenCode: [
      { required: true, message: "隐码值不能为空", trigger: "blur" }
    ],
    belongOrgId: [
...
const getList = async () => {
  loading.value = true;
  const res = await listHiddenCodeDetail(queryParams.value);
  hiddenCodeDetailList.value = res.rows;
...
const reset = () => {
  ...
  hiddenCodeDetailFormRef.value?.resetFields();
}
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: HiddenCodeDetailVO[]) => {
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "添加隐码变量详情对照";
}

/** 修改按钮操作 */
const handleUpdate = async (row?: HiddenCodeDetailVO) => {
  reset();
  const _id = row?.id || ids.value[0]
  const res = await getHiddenCodeDetail(_id);
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = "修改隐码变量详情对照";
}

/** 提交按钮 */
const submitForm = () => {
  hiddenCodeDetailFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.id) {
        await updateHiddenCodeDetail(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        await addHiddenCodeDetail(form.value).finally(() =>  buttonLoading.value = false);
      }
...
/** 删除按钮操作 */
const handleDelete = async (row?: HiddenCodeDetailVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认删除隐码变量详情对照编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delHiddenCodeDetail(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('mes/hiddenCodeDetail/export', {
    ...queryParams.value
  }, `hiddenCodeDetail_${new Date().getTime()}.xlsx`)
}
