<template>
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="隐码变量" prop="hiddenCodeGroup">
              <el-input v-model="queryParams.hiddenCodeGroup" placeholder="请输入隐码变量" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:hiddenCode:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:hiddenCode:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:hiddenCode:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:hiddenCode:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="hiddenCodeList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="隐码变量" align="center" prop="hiddenCodeGroup" />
        <el-table-column label="描述" align="center" prop="description" />
        <el-table-column label="状态, ENABLED|DISABLED" align="center" prop="status" />
        <el-table-column label="隐码类型,年:YEAR,月:MONTH,天:DAY" align="center" prop="groupType" />
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:hiddenCode:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:hiddenCode:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="hiddenCodeFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="数据行索引" prop="handle">
          <el-input v-model="form.handle" placeholder="请输入数据行索引" />
        </el-form-item>
        <el-form-item label="隐码变量" prop="hiddenCodeGroup">
          <el-input v-model="form.hiddenCodeGroup" placeholder="请输入隐码变量" />
        </el-form-item>
        <el-form-item label="描述" prop="description">
...
<script setup name="HiddenCode" lang="ts">
import { listHiddenCode, getHiddenCode, delHiddenCode, addHiddenCode, updateHiddenCode } from '@/api/mes/hiddenCode';
import { HiddenCodeVO, HiddenCodeQuery, HiddenCodeForm } from '@/api/mes/hiddenCode/types';
...
const hiddenCodeFormRef = ref<ElFormInstance>();
...
const initFormData: HiddenCodeForm = {
  id: undefined,
  handle: undefined,
...
const data = reactive<PageData<HiddenCodeForm, HiddenCodeQuery>>({
  form: {...initFormData},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
  ...
  rules: {
    ...
    hiddenCodeGroup: [
      { required: true, message: "隐码变量不能为空", trigger: "blur" }
    ],
    description: [
      { required: true, message: "描述不能为空", trigger: "blur" }
    ],
    status: [
      { required: true, message: "状态, ENABLED|DISABLED不能为空", trigger: "change" }
    ],
    groupType: [
      { required: true, message: "隐码类型,年:YEAR,月:MONTH,天:DAY不能为空", trigger: "change" }
    ],
    belongOrgId: [
...
const getList = async () => {
  loading.value = true;
  const res = await listHiddenCode(queryParams.value);
...
const reset = () => {
  ...
  hiddenCodeFormRef.value?.resetFields();
}
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "添加隐码变量对照";
}

/** 修改按钮操作 */
const handleUpdate = async (row?: HiddenCodeVO) => {
  reset();
  const _id = row?.id || ids.value[0]
  const res = await getHiddenCode(_id);
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = "修改隐码变量对照";
}

/** 提交按钮 */
const submitForm = () => {
  hiddenCodeFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.id) {
        await updateHiddenCode(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        await addHiddenCode(form.value).finally(() =>  buttonLoading.value = false);
      }
...
const handleDelete = async (row?: HiddenCodeVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除隐码变量对照编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delHiddenCode(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('mes/hiddenCode/export', {
    ...queryParams.value
  }, `hiddenCode_${new Date().getTime()}.xlsx`)
}
