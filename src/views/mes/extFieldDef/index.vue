<template>
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="扩展字段关联的表" prop="tableName">
              <el-input v-model="queryParams.tableName" placeholder="请输入扩展字段关联的表" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="定义扩展字段的编码" prop="fieldName">
              <el-input v-model="queryParams.fieldName" placeholder="请输入定义扩展字段的编码" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['mes:extFieldDef:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['mes:extFieldDef:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['mes:extFieldDef:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['mes:extFieldDef:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="extFieldDefList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="扩展字段关联的表" align="center" prop="tableName" />
        <el-table-column label="定义扩展字段的编码" align="center" prop="fieldName" />
        <el-table-column label="定义扩展字段的描述" align="center" prop="description" />
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['mes:extFieldDef:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['mes:extFieldDef:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="extFieldDefFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="数据行索引" prop="handle">
          <el-input v-model="form.handle" placeholder="请输入数据行索引" />
        </el-form-item>
        <el-form-item label="扩展字段关联的表" prop="tableName">
          <el-input v-model="form.tableName" placeholder="请输入扩展字段关联的表" />
        </el-form-item>
        <el-form-item label="定义扩展字段的编码" prop="fieldName">
          <el-input v-model="form.fieldName" placeholder="请输入定义扩展字段的编码" />
        </el-form-item>
        <el-form-item label="定义扩展字段的描述" prop="description">
          <el-input v-model="form.description" placeholder="请输入定义扩展字段的描述" />
        </el-form-item>
        <el-form-item label="顺序号" prop="sequence">
...
<script setup name="ExtFieldDef" lang="ts">
import { listExtFieldDef, getExtFieldDef, delExtFieldDef, addExtFieldDef, updateExtFieldDef } from '@/api/mes/extFieldDef';
import { ExtFieldDefVO, ExtFieldDefQuery, ExtFieldDefForm } from '@/api/mes/extFieldDef/types';

const { proxy } = getCurrentInstance() as ComponentInternalInstance;

const extFieldDefList = ref<ExtFieldDefVO[]>([]);
...
const extFieldDefFormRef = ref<ElFormInstance>();
...
const initFormData: ExtFieldDefForm = {
  id: undefined,
  handle: undefined,
  tableName: undefined,
  fieldName: undefined,
...
const data = reactive<PageData<ExtFieldDefForm, ExtFieldDefQuery>>({
  form: {...initFormData},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
  ...
  rules: {
    ...
    tableName: [
      { required: true, message: "扩展字段关联的表不能为空", trigger: "blur" }
    ],
    fieldName: [
      { required: true, message: "定义扩展字段的编码不能为空", trigger: "blur" }
    ],
    description: [
      { required: true, message: "定义扩展字段的描述不能为空", trigger: "blur" }
    ],
    sequence: [
...
const getList = async () => {
  loading.value = true;
  const res = await listExtFieldDef(queryParams.value);
  extFieldDefList.value = res.rows;
...
const reset = () => {
  ...
  extFieldDefFormRef.value?.resetFields();
}
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: ExtFieldDefVO[]) => {
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "添加主数据扩展字段定义";
}

/** 修改按钮操作 */
const handleUpdate = async (row?: ExtFieldDefVO) => {
  reset();
  const _id = row?.id || ids.value[0]
  const res = await getExtFieldDef(_id);
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = "修改主数据扩展字段定义";
}

/** 提交按钮 */
const submitForm = () => {
  extFieldDefFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.id) {
        await updateExtFieldDef(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        await addExtFieldDef(form.value).finally(() =>  buttonLoading.value = false);
      }
...
/** 删除按钮操作 */
const handleDelete = async (row?: ExtFieldDefVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认删除主数据扩展字段定义编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delExtFieldDef(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('mes/extFieldDef/export', {
    ...queryParams.value
  }, `extFieldDef_${new Date().getTime()}.xlsx`)
}
