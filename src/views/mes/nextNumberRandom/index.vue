  ...
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="行号" prop="handle">
              <el-input v-model="queryParams.handle" placeholder="请输入行号" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="租户ID" prop="site">
              <el-input v-model="queryParams.site" placeholder="请输入租户ID" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            ...
            <el-form-item label="随机数范围" prop="letterRange">
              <el-input v-model="queryParams.letterRange" placeholder="请输入随机数范围" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="nextNumberRandomList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="行号" align="center" prop="handle" />
        <el-table-column label="租户ID" align="center" prop="site" />
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="nextNumberRandomFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="行号" prop="handle">
          <el-input v-model="form.handle" placeholder="请输入行号" />
        </el-form-item>
        <el-form-item label="租户ID" prop="site">
          <el-input v-model="form.site" placeholder="请输入租户ID" />
        </el-form-item>
        <el-form-item label="随机数编号" prop="randomCode">
...
const data = reactive<PageData<NextNumberRandomForm, NextNumberRandomQuery>>({
  ...
  rules: {
    ...
    randomCode: [
      { required: true, message: "随机数编号不能为空", trigger: "blur" }
    ],
    description: [
      { required: true, message: "描述不能为空", trigger: "blur" }
    ],
    letterRange: [
      { required: true, message: "随机数范围不能为空", trigger: "blur" }
    ],
    belongOrgId: [
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('mes/nextNumberRandom/export', {
    ...queryParams.value
