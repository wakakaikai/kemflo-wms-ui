<template>
  <div class="serial-debugger weight-before-container">
...
const SCAN_WEIGHT_BEFORE_KEY = 'workPanelTestBeforeWeight';
const loadScanBeforeWeight = () => {
  try {
    const cache = JSON.parse(localStorage.getItem(SCAN_WEIGHT_BEFORE_KEY) || '{}');
...
const saveScanBeforeWeight = () => {
  ...
  localStorage.setItem(SCAN_WEIGHT_BEFORE_KEY, JSON.stringify(cache));
};
...
onMounted(() => {
  ...
  const resOperationObj = getOperationFromLocalStorage();
  if (!resOperationObj) {
    if (lastSegment === 'WGT-BF' || lastSegment === 'weight-before') {
