...
const getDataSource = async () => {
  ...
  loading.value = true;
  const res: any = await services.getDataCollectionBySfc({
      ...
      firstArray.forEach((data: any) => {
        if (data.dataCollectionDetailVOList && data.dataCollectionDetailVOList.length > 0) {
          data.dataCollectionDetailVOList.forEach((item: any) => {
            item.dcGroupRef = data.dcGroupRef;
            item.dcGroup = data.dcGroup;
            item.dcGroupRevision = data.dcGroupRevision;
            item.firstDescription = data.description;
            item.dcGroupAndRevision = `${data.dcGroup}/${data.dcGroupRevision}`;
            item.id = idCounter++;
...
const handleOk = async () => {
  const params = {
    activityId: props.podFormdata.activity,
    sfc: formData.sfc,
