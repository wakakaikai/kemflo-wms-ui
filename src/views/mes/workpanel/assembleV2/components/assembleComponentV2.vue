<template>
  <el-card>
    ...
    <el-table v-loading="loading" :data="form.bomComponentVoList" row-key="id" border highlight-current-row :row-class-name="({ row }) => (row.validateMsg ? 'error-row' : '')">
      ...
      <el-table-column prop="bomItemSfc" label="组件条码" min-width="300" align="center">
        <template #default="{ row, $index }">
          <div class="table-cell-wrapper">
            <el-form-item :prop="`bomItemSfc${$index}`" :class="['dynamic-error-item', { 'has-error': !!row.validateMsg }]">
              <el-input
              ...
              />
              <div v-if="row.validateMsg" class="custom-error-msg">
                {{ row.validateMsg }}
              </div>
            </el-form-item>
          </div>
      ...
      <el-table-column label="组件类型" align="center" prop="componentSnType">
        <template #default="scope">
          <dict-tag :options="mes_bom_component_type" :value="scope.row.componentSnType" />
        </template>
      </el-table-column>
      <!-- 操作列 -->
    ...
    <ReplaceComponentV2Dialog ref="replaceComponentV2DialogRef" @replace-component-call-back="replaceComponentCallBack" />
  </el-card>
</template>
...
import ReplaceComponentV2Dialog from '@/views/mes/workpanel/assembleV2/components/replaceComponentV2Dialog.vue';
const replaceComponentV2DialogRef = ref<InstanceType<typeof ReplaceComponentV2Dialog>>();
...
const querySfcBomComponentList = async () => {
  const res: any = await services.querySfcShopOrderBomComponentList({
    sfc: form.value?.sfc,
  ...
  if (res.code == HttpStatus.SUCCESS) {
    if (res.data.length == 0) {
      proxy?.$modal.msgError('待装配列表为空，请检查工序是否正确或联系IE检查关键件BOM。');
      return;
    }
    form.value.bomComponentVoList = res.data?.map((item: any, index: any) => ({
      ...item,
      [`bomItemSfc${index}`]: item.fixedSfc || item.bomItemSfc,
      validateStatus: '',
      validateMsg: ''
    }));
...
const onReplace = (record: any) => {
  ...
  replaceComponentV2DialogRef.value.openDialog({
...
const onDisassemble = (record: any) => {
  // publicRecord.components = record.components;
  // publicRecord.componentAndRevision = `${record.components}/${record.componentRevision}`;
