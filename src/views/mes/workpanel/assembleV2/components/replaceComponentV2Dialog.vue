  ...
  <el-dialog v-model="visible" :title="title" width="70%" append-to-body @opened="handleDialogOpened">
    <el-form v-loading="loading" ref="queryFormRef" :model="form" :rules="rules" label-width="120px" class="optimized-form">
      <el-row :gutter="20">
        ...
        <el-col :lg="12" :md="12" :xs="24" v-if="form.fixedSfc"> </el-col>
        <el-col :lg="12" :md="12" :xs="24" v-if="form.fixedSfc">
          <el-form-item label="固定条码：" class="form-item-content deleting-notice">
            <el-text class="form-text deleting-notice">{{ form.fixedSfc }}</el-text>
          </el-form-item>
        </el-col>
        <el-col :lg="12" :md="12" :xs="24">
        ...
        <el-col :lg="12" :md="12" :xs="24">
          <el-form-item label="替换组件条码：" prop="newBomItemSfc" :class="['dynamic-error-item', { 'has-error': !!validateResult.validateMsg }]">
            ...
            <div v-if="validateResult.validateMsg" class="custom-error-msg">
              {{ validateResult.validateMsg }}
            </div>
          </el-form-item>
        </el-col>
      </el-row>
  ...
  queryFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      const params = {
        ...
        componentSnType: form.value.componentSnType,
        activityId: ''
      };
      loading.value = true;
