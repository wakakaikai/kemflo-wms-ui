    ...
    <el-card shadow="never" class="transfer-main-card">
      <template #header>
        <div class="transfer-header">
          <span class="header-title">领料列表</span>
          <div class="header-actions">
            ...
            <el-button type="danger" @click="clearIssueList" :disabled="issueList.length === 0">清空</el-button>
            <right-toolbar :search="false" :columns="issueColumns" />
          </div>
        </div>
      </template>

      <div class="transfer-form-bar">
        <el-form :model="issueForm" label-width="auto" :inline="true">
          <el-row :gutter="12">
            <el-col :sm="24" :md="8" :lg="8">
              <el-form-item label="物料单">
                <HistoryInput v-model="issueForm.mtsnr" :config="mtsnrConfig" placeholder="请输入物料单" />
              </el-form-item>
            </el-col>
            <el-col :sm="24" :md="7" :lg="7">
              <el-form-item label="凭证抬头文本">
                <HistoryInput v-model="issueForm.bktxt" :config="bktxtConfig" placeholder="请输入凭证抬头文本" />
              </el-form-item>
            </el-col>
            <el-col :sm="24" :md="1" :lg="1" class="transfer-more-toggle-col">
              <el-form-item label-width="0">
                <el-button link type="primary" @click="showIssueMore = !showIssueMore">
                  <el-icon class="el-icon--right">
                    <ArrowDown v-if="!showIssueMore" />
                    <ArrowUp v-else />
                  </el-icon>
                </el-button>
              </el-form-item>
          ...
          <el-row v-show="showIssueMore" :gutter="12" class="transfer-form-more">
            <el-col :sm="24" :md="8" :lg="8">
              <el-form-item label="过账日期" prop="postingDate">
                <el-date-picker clearable v-model="issueForm.postingDate" type="date" :disabled-date="disabledFutureDate" value-format="YYYY-MM-DD" placeholder="请选择过账日期" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
      ...
      <el-table :data="issueList" border class="transfer-table" v-loading="tableLoading" :max-height="issueTableHeight">
        <el-table-column type="index" width="40" align="center" />
        <el-table-column v-if="issueColumns[0].visible" label="物料编码" prop="itemCode" min-width="100" show-overflow-tooltip />
        <el-table-column v-if="issueColumns[1].visible" label="物料名称" prop="itemName" min-width="120" show-overflow-tooltip />
        <el-table-column v-if="issueColumns[2].visible" label="批次号" prop="batchCode" min-width="90" show-overflow-tooltip />
        <el-table-column v-if="issueColumns[3].visible" label="源库位信息" width="180">
          <template #default="scope">
            <div>
        ...
        <el-table-column v-if="issueColumns[4].visible" label="数量" min-width="180">
          <template #default="scope">
            <div>
        ...
        <el-table-column v-if="issueColumns[5].visible" label="单位" prop="unit" align="center" width="80" />
        <el-table-column v-if="issueColumns[6].visible" label="库存标识" align="center" prop="specialInventoryFlag" min-width="90">
          <template #default="scope">
            <dict-tag :options="wms_inventory_special_flag" :value="scope.row.specialInventoryFlag" />
          </template>
        </el-table-column>
        <el-table-column v-if="issueColumns[7].visible && showBusinessPartnerColumn" label="业务伙伴" align="center" min-width="110">
          <template #default="scope">
        ...
        <el-table-column v-if="issueColumns[8].visible" label="库存类型" prop="inventoryType" align="center" min-width="110">
          <template #default="scope">
        ...
        <el-table-column label="成本中心" min-width="200">
          <template #default="scope">
        ...
        <el-table-column v-if="issueColumns[9].visible" label="领料数量" width="115">
          <template #default="scope">
        ...
        <el-table-column v-if="issueColumns[10].visible && showMoveReasonColumn" label="移动原因" min-width="180">
          <template #default="scope">
      ...
      <div class="submit-bar">
        <el-button :loading="buttonLoading" type="primary" @click="submitIssue" :disabled="issueList.length === 0">{{ moveType }} {{ moveTypeDesc }}</el-button>
        <span class="transfer-count">共 {{ issueList.length }} 条</span>
      </div>
    </el-card>
...
<script setup name="DeptMaterialIssue" lang="ts">
...
import { listCostCenterMoveType } from '@/api/wms/costCenterMoveType';
import type { CostCenterMoveTypeVO } from '@/api/wms/costCenterMoveType/types';
import type { MoveTypeReasonVO } from '@/api/wms/moveTypeReason/types';
import { DEFAULT_DEPT_ISSUE_MOVE_TYPE, DEPT_ISSUE_MOVE_TYPES, getDefaultSourceInventoryType, getIssueMoveTypeDesc, isDeptIssueZ01 } from './utils/issueMoveConfig';
...
const issueTableHeight = 'calc(100vh - 320px)';
...
const moveType = ref(DEFAULT_DEPT_ISSUE_MOVE_TYPE);
const moveTypeReasonOptions = ref<MoveTypeReasonVO[]>([]);
const moveTypeReasonLoading = ref(false);
const skipSap = ref(false);
const moveTypeOptions = DEPT_ISSUE_MOVE_TYPES;
const moveTypeDesc = computed(() => getIssueMoveTypeDesc(moveType.value));
const isDeptIssueZ01Move = computed(() => isDeptIssueZ01(moveType.value));
const costCenterOptions = ref<CostCenterMoveTypeVO[]>([]);
const costCenterLoading = ref(false);
const showBusinessPartnerColumn = computed(() => issueList.value.some((item) => item.specialInventoryFlag && item.specialInventoryFlag !== 'N'));
...
const historyPage = 'deptMaterialIssue';
...
const issueColumns = ref<FieldOption[]>([
  { key: 0, label: `物料编码`, visible: true, children: [] },
  { key: 1, label: `物料名称`, visible: true, children: [] },
  { key: 2, label: `批次号`, visible: true, children: [] },
  ...
  { key: 9, label: `领料数量`, visible: true, children: [] },
...
const addSelectedToIssueList = () => {
  ...
  newItems.forEach((newItem) => {
    issueList.value.push(newItem);
    addedCount++;
  });

  proxy.$modal.msgSuccess(`成功添加${addedCount}条记录到领料列表`);
};
...
const handleMoveTypeChange = async () => {
  resultMessage.value = '';
  await Promise.all([loadMoveTypeReasonOptions(), loadCostCenterOptions()]);
  const allowedCostCenters = new Set(costCenterOptions.value.map((item) => item.costCenter));
  const sourceInventoryType = getDefaultSourceInventoryType();
  issueList.value.forEach((item) => {
    ...
    item.moveReasonCode = '';
    item.moveReasonDesc = '';
    if (!allowedCostCenters.has(item.costCenter)) {
      item.costCenter = '';
    }
    if (!isDeptIssueZ01Move.value) {
      item.orderNo = '';
    }
  });
};
...
const submitIssue = async () => {
  const validItems = issueList.value.filter((item) => item.transferQuantity > 0);
  resultStatus.value = true;
  resultMessage.value = '';
  if (validItems.length === 0) {
    resultMessage.value = '没有有效的领料记录';
    resultStatus.value = false;
    return;
  }

  const missingCostCenter = validItems.filter((item) => !String(item.costCenter || '').trim());
  ...
  if (overQuantityItems.length > 0) {
    resultMessage.value = '领料数量不能超过当前可用数量';
    resultStatus.value = false;
    return;
  }
  ...
  try {
    ...
    const res: any = await transferInventory({
      ...
      transferType: 0,
      moveType: moveType.value,
      mtsnr: resolveMtsnr(issueForm.value.mtsnr),
      bktxt: resolveBatchBktxt(issueForm.value.bktxt),
      postingDate: formatPostingDate(issueForm.value.postingDate),
    ...
    resultMessage.value = res.msg || `成功领料${transferRequests.length}条记录`;
    resultStatus.value = true;
    issueList.value = [];
    issueForm.value.mtsnr = '';
    issueForm.value.bktxt = '';
    issueForm.value.postingDate = null;
    handleQuery();
  } catch (error: any) {
    resultMessage.value = error.message || '领料失败';
    resultStatus.value = false;
  } finally {
