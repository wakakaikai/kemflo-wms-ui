<template>
  <div class="p-2 prep-demand-page">
    ...
    <el-tabs v-model="statusTab" class="status-tabs" @tab-change="onStatusTabChange">
      ...
      <el-tab-pane label="缺料关注" name="shortage" />
    </el-tabs>
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true" label-width="auto">
            ...
            <el-form-item label="状态" prop="demandStatus">
              <el-select v-model="queryParams.demandStatus" placeholder="全部" clearable style="width: 140px">
                <el-option v-for="opt in DEMAND_STATUS_OPTIONS" :key="opt.value" :label="opt.label" :value="opt.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="库存检查" prop="checkPassed">
              <el-select v-model="queryParams.checkPassed" placeholder="全部" clearable style="width: 120px">
      ...
      <div v-loading="loading">
        ...
        <template v-else>
          <el-table :data="prepDemandList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="需求单号" align="center" prop="demandNo" min-width="150">
              <template #default="{ row }"><el-link type="primary" @click="openDetail(row.id)">{{ row.demandNo }}</el-link></template>
            </el-table-column>
            <el-table-column label="状态" align="center" prop="demandStatus" width="100">
              <template #default="{ row }"><el-tag :type="demandStatusTag(row.demandStatus)" size="small">{{ demandStatusLabel(row.demandStatus) }}</el-tag></template>
            </el-table-column>
            <el-table-column label="齐套率" align="center" width="120">
              <template #default="{ row }">
                <div class="table-kit">
                  <span :style="{ color: kitRateColor(row.kitRate) }">{{ formatKitRate(row.kitRate) }}%</span>
                  <el-progress :percentage="kitRatePercentValue(row.kitRate)" :color="kitRateColor(row.kitRate)" :stroke-width="6" :show-text="false" />
                </div>
              </template>
            </el-table-column>
            <el-table-column label="工单数" align="center" prop="workOrderCount" width="80" />
            <el-table-column label="库存检查" align="center" width="90">
              <template #default="{ row }"><el-tag :type="row.checkPassed ? 'success' : 'warning'" size="small">{{ row.checkPassed ? '通过' : '未通过' }}</el-tag></template>
            </el-table-column>
            <el-table-column label="待发/可用/缺料" align="center" min-width="150">
              <template #default="{ row }">{{ row.totalRequired ?? '-' }} / {{ row.totalAvailable ?? '-' }} / {{ row.totalShortage ?? '-' }}</template>
            </el-table-column>
            <el-table-column label="紧急" align="center" width="70">
              <template #default="{ row }"><el-tag v-if="row.isEmergency" type="danger" size="small">是</el-tag><span v-else>-</span></template>
            </el-table-column>
            <el-table-column label="创建时间" align="center" prop="createTime" width="160">
              <template #default="{ row }"><span>{{ parseTime(row.createTime, '{y}-{m}-{d} {h}:{i}') }}</span></template>
            </el-table-column>
            <el-table-column label="操作" align="center" width="200" fixed="right">
              <template #default="{ row }">
                <el-button link type="primary" @click="openDetail(row.id)" v-hasPermi="['wms:workOrderPrepDemand:query']">详情</el-button>
                <el-button v-if="row.issueId" link type="success" @click="openIssue(row.issueId)" v-hasPermi="['wms:materialIssue:query']">领料</el-button>
...
const initFormData: WorkOrderPrepDemandForm = { id: undefined, demandNo: undefined, demandStatus: undefined, isEmergency: false, emergencyLevel: undefined, emergencyReason: undefined, remark: undefined };
const data = reactive<PageData<WorkOrderPrepDemandForm, WorkOrderPrepDemandQuery>>({
  form: { ...initFormData },
  queryParams: { pageNum: 1, pageSize: 12, demandNo: undefined, demandStatus: undefined, checkPassed: undefined, isEmergency: undefined, params: {} },
...
const pageAvgKitRate = computed(() => {
  ...
  const sum = prepDemandList.value.reduce((acc, row) => acc + kitRatePercentValue(row.kitRate), 0);
...
const onStatusTabChange = (tab: string | number) => {
  ...
  if (name === 'all') {
    ...
    queryParams.value.demandStatus = name;
    queryParams.value.checkPassed = undefined;
  }
...
const openDetail = (id: number | string) => { currentDemandId.value = id; detailVisible.value = true; };
const openIssue = (issueId: number | string) => { currentIssueId.value = issueId; issueDrawerVisible.value = true; };
const fillFormFromDemand = (demand: WorkOrderPrepDemandVO) => {
  form.value = { id: demand.id, demandNo: demand.demandNo, demandStatus: demand.demandStatus, isEmergency: demand.isEmergency ?? false, emergencyLevel: demand.emergencyLevel, emergencyReason: demand.emergencyReason, remark: demand.remark };
};
...
const openEditFromDetail = (demand: WorkOrderPrepDemandVO) => { reset(); fillFormFromDemand(demand); dialog.visible = true; dialog.title = '修改备料需求'; };
const submitForm = () => {
...
const handleExport = () => { syncDateRange(); proxy?.download('wms/workOrderPrepDemand/export', { ...queryParams.value }, `workOrderPrepDemand_${Date.now()}.xlsx`); };
onMounted(() => { getList(); });
</script>
