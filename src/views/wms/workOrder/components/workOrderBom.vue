  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="workOrderBomList" @selection-change="handleSelectionChange">
        ...
        <el-table-column v-if="columns[1].visible" label="预留单号" align="center" prop="reserveNo" min-width="130" />
        <el-table-column v-if="columns[2].visible" label="预留单项次" align="center" prop="reserveItemNo" min-width="100" />
        <el-table-column v-if="columns[3].visible" label="组件料号" align="center" prop="componentMaterial" min-width="150" />
        <el-table-column v-if="columns[4].visible" label="组件描述" align="left" prop="componentDesc" show-overflow-tooltip min-width="300" />
        <el-table-column v-if="columns[5].visible" label="需求数量" align="center" prop="componentQty" min-width="120" />
        <el-table-column v-if="columns[6].visible" label="基本单位" align="center" prop="inventoryUnit" />
        <el-table-column v-if="columns[7].visible" label="数量" align="center" prop="issueUomQty" min-width="120" />
        <el-table-column v-if="columns[8].visible" label="单位" align="center" prop="unit" />
        <el-table-column v-if="columns[9].visible" label="已发料数量" align="center" prop="issuedQty" min-width="120" />
        <el-table-column v-if="columns[10].visible" label="换算比例" align="center" prop="conversionRatio" />
        <el-table-column v-if="columns[11].visible" label="创建时间" align="center" prop="createTime" />
        <el-table-column v-if="columns[12].visible" label="创建者" align="center" prop="createByName" />
        <el-table-column v-if="columns[13].visible" label="更新时间" align="center" prop="updateTime" />
        <el-table-column v-if="columns[14].visible" label="更新者" align="center" prop="updateByName" />
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="workOrderBomFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="工单号" prop="workOrderNo">
          <el-input v-model="form.workOrderNo" placeholder="请输入工单号" :readonly="!!workOrderNo" />
        </el-form-item>
        <el-form-item label="组件料号" prop="componentMaterial">
        ...
        <el-form-item label="基本单位" prop="inventoryUnit">
          <el-input v-model="form.inventoryUnit" placeholder="请输入基本单位" />
        </el-form-item>
...
const columns = ref<FieldOption[]>([
  ...
  { key: 1, label: `预留单号`, visible: true, children: [] },
  { key: 2, label: `预留单号`, visible: true, children: [] },
  { key: 3, label: `组件料号`, visible: true, children: [] },
  { key: 4, label: `组件描述`, visible: true, children: [] },
  { key: 5, label: `需求数量`, visible: true, children: [] },
  { key: 6, label: `基本单位`, visible: true, children: [] },
  { key: 7, label: `数量`, visible: true, children: [] },
  { key: 8, label: `单位`, visible: true, children: [] },
  { key: 9, label: `已发料数量`, visible: true, children: [] },
  { key: 10, label: `换算比例`, visible: true, children: [] },
  { key: 11, label: `创建时间`, visible: false, children: [] },
  { key: 12, label: `创建者`, visible: false, children: [] },
  { key: 13, label: `更新时间`, visible: false, children: [] },
  { key: 14, label: `更新者`, visible: false, children: [] },
  { key: 15, label: `备注`, visible: false, children: [] }
]);
...
/** 表单重置 */
const reset = () => {
  form.value = { ...initFormData, workOrderNo: props.workOrderNo };
...
const handleAdd = () => {
  reset();
  // 设置工单号为父组件传递的值
  form.value.workOrderNo = props.workOrderNo;
  dialog.visible = true;
