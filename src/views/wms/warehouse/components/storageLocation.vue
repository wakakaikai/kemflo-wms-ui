<template>
  <div class="p-2">
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="storageLocationFormRef" :model="form" :rules="rules" label-width="auto">
        <el-form-item label="库位编码" prop="locationCode">
...
const data = reactive<PageData<StorageLocationForm, StorageLocationQuery>>({
  ...
  rules: {
    ...
    areaId: [{ required: true, message: '库区ID不能为空', trigger: 'blur' }]
  }
});
...
onMounted(() => {
  if (route.query && (route.query.areaId as string)) {
    let areaId: string | number;

    if (Array.isArray(route.query.areaId)) {
      // 如果是数组，取第一个元素
      areaId = route.query.areaId[0];
    } else {
      // 如果是字符串，直接使用
    ...
    form.value.areaId = areaId;
    initFormData.areaId = areaId;
  }
  getList();
});
</script>
