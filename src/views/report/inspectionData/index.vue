        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :rules="rules" :inline="true" label-width="auto">
            <el-form-item label="资源" prop="resource">
              <el-input v-model="queryParams.resource" placeholder="请输入或选择资源" clearable @keyup.enter="handleQuery">
                <template #append>
                  <el-button icon="Search" @click="openResourceDialog" />
                </template>
              </el-input>
            </el-form-item>
            <el-form-item label="巡检时间" prop="dateTimeRange">
              <el-date-picker
...
import { listMesInspectionData } from '@/api/report/mesProductData';
import { MesProductDataForm, MesProductDataQuery } from '@/api/report/mesProductData/types';
import { ref } from 'vue';
...
const data = reactive<PageData<MesProductDataForm, MesProductDataQuery>>({
  ...
  rules: {
    resource: [{ required: true, message: '资源不能为空' }]
  }
});
...
const resourceCallBack = (data: any) => {
  queryParams.value.resource = data.resrce;
  queryParams.value.resourceDesc = data.description;
};
/** 查询MES生产数据列表 */
const getList = async () => {
  mesProductDataList.value = [];
  loading.value = true;
  const res = await listMesInspectionData(queryParams.value);
...
const handleExport = () => {
  proxy?.download(
    'wms/mes/inspection/export',
    {
      ...queryParams.value
    },
    `MES巡检数据_${new Date().getTime()}.xlsx`
  );
};
