<template>
  <div ref="boardRef" class="exception-dashboard">
    ...
    <div class="production-list">
      <!--        <el-carousel :autoplay="shouldAutoPlay" height="auto" indicator-position="outside" :interval="5000" arrow="always">-->
      <el-carousel :autoplay="false" height="auto" indicator-position="outside" :interval="5000" arrow="always">
        <el-carousel-item v-for="(chunk, index) in chunkedProductionLines" :key="index" :style="{ height: carouselItemHeight }">
          <!-- 表头 -->
          <!--          <div class="production-header">
            <div class="header-cell line-header">线体/工序</div>
            <div v-for="processKey in displayedProcesses" :key="processKey" class="header-cell">
              {{ processNameMap[processKey] }}
            </div>
          </div>-->

          <!-- 生产线数据 -->
          <div v-for="line in chunk" :key="line.line" class="line-row">
            <!-- 线体名称 -->
            <div class="line-name">{{ line.line }}</div>

            <!-- 工序状态 -->
            <div
              v-for="processKey in displayedProcesses"
              :key="processKey"
              class="station-cell"
              :class="{
                abnormal: isAbnormal(line.processes[processKey]),
                empty: !line.processes[processKey],
                'first-process': displayedProcesses[0] === processKey
              }"
            >
...
const chunkedProductionLines = computed(() => {
  const chunkSize = 6; // 每页最多显示5条产线
  const maxFirstRowProcesses = 6; // 第一行最多6个工序
  const maxOtherRowProcesses = 7; // 其他行每行最多7个工序

  const chunks = [];
  let currentChunk = [];

  // 计算每条产线需要的行数
  const calculateLinesNeeded = (processes) => {
    if (!processes || Object.keys(processes).length === 0) return 0;
    const processCount = Object.keys(processes).length;
    return processCount <= maxFirstRowProcesses ? 1 : Math.ceil((processCount - maxFirstRowProcesses) / maxOtherRowProcesses) + 1;
  };

  let currentChunkLines = 0;

  for (const line of productionLines.value) {
    const linesNeeded = calculateLinesNeeded(line.processes);

    // 如果添加当前产线会超过chunkSize限制，就保存当前chunk并新建一个
    if (currentChunk.length > 0 && currentChunkLines + linesNeeded > chunkSize) {
...
const shouldAutoPlay = computed(() => {
  return chunkedProductionLines.value.length > 1;
});

const carouselItemHeight = computed(() => {
  if (isFullscreen.value) {
    // 全屏模式下使用calc计算高度，减去必要的间距
    return 'calc(64vh + 10px)';
  } else {
    // 普通模式保持原有计算逻辑
    const headerHeight = 160; // 表头高度
    const rowHeight = 80; // 每行高度
    const rows = chunkedProductionLines.value[0]?.length || 0;
    return `${headerHeight + rows * rowHeight + 20}px`;
  }
});
