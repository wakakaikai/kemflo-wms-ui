<template>
  <div v-loading="state.loading" class="layout-navbars-breadcrumb-user-news">
    ...
    <div v-if="newsList.length > 0" class="foot-box" @click="onGoToGiteeClick">前往呼叫看板</div>
  </div>
</template>
...
//点击消息，写入已读
const onNewsClick = async (index: any) => {
  try {
    state.loading = true;
    const noticeId = newsList.value[index].id;
