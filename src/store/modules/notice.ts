...
export const useNoticeStore = defineStore('notice', () => {
  ...
  const saveToDB = async (): Promise<void> => {
    try {
      ...
      await localforage.setItem('notices', serialized);
    } catch (error) {
  ...
  const loadFromDB = async (): Promise<void> => {
    ...
    try {
      const saved = await localforage.getItem<NoticeItem[]>('notices');
      if (saved) {
  ...
  // 添加新通知
  const addNotice = async (notice: Omit<NoticeItem, 'id' | 'time' | 'read'>): Promise<NoticeItem> => {
    ...
    // 根据优先级插入到合适位置
    const index = state.notices.findIndex((n) => n.priority < newNotice.priority);
    if (index === -1) {
      state.notices.push(newNotice);
    } else {
      state.notices.splice(index, 0, newNotice);
    }
